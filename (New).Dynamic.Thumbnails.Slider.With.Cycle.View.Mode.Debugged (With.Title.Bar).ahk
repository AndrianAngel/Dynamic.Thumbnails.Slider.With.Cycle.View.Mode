; AutoHotkey v1 Script - DEBUGGED VERSION
#SingleInstance Force

; Global variables
global viewModeIndex := 0
global viewModes := ["LargeThumbs", "Tiles", "Details", "List"]
global guiVisible := false
global debugMode := false  ; Set to false to disable debug messages

; --- GUI Setup ---
Gui, +AlwaysOnTop +ToolWindow
Gui, Color, 0x282D41

hBitmap := Create_sliderthumb_png()
slider := new CustomSlider("y20 w400 h3 BackGround0x454C5F c0x00BFFF Range1-100", 20, hBitmap, true)

; --- Real-time size display ---
Gui, Add, Text, x430 y15 w120 h24 vSizeDisplay Center c0x00BFFF BackgroundTrans, Size: 96px

; --- Add buttons for view modes ---
Gui, Add, Button, x20  y50 w120 h30 gSetLargeThumbs HwndHBtn1, Large Thumbnails
Gui, Add, Button, x160 y50 w80  h30 gSetTiles HwndHBtn2, Tiles
Gui, Add, Button, x260 y50 w80  h30 gSetDetails HwndHBtn3, Details
Gui, Add, Button, x360 y50 w80  h30 gSetList HwndHBtn4, List
Gui, Add, Button, x460 y50 w80  h30 gSetCycleViewMode HwndHBtn5, Cycle

DllCall("UxTheme\SetWindowTheme", "Ptr", hBtn1, "Str", "DarkMode_Explorer", "Ptr", 0)
DllCall("UxTheme\SetWindowTheme", "Ptr", hBtn2, "Str", "DarkMode_Explorer", "Ptr", 0)
DllCall("UxTheme\SetWindowTheme", "Ptr", hBtn3, "Str", "DarkMode_Explorer", "Ptr", 0)
DllCall("UxTheme\SetWindowTheme", "Ptr", hBtn4, "Str", "DarkMode_Explorer", "Ptr", 0)
DllCall("UxTheme\SetWindowTheme", "Ptr", hBtn5, "Str", "DarkMode_Explorer", "Ptr", 0)

Gui, Show, h100 w560 NA
Gui, Hide   ; start hidden

; --- Hotkeys ---
^F12::ToggleGui()
^w::CycleViewModeHotkey()
^d::ToggleDebugMode()  ; Toggle debug messages

; Emergency exit
Esc::ExitApp

; --- Toggle Debug Mode ---
ToggleDebugMode() {
    global debugMode
    debugMode := !debugMode
    ShowNotification("Debug Mode: " (debugMode ? "ON" : "OFF"))
}

; --- Debug Logger ---
DebugLog(message) {
    global debugMode
    if (debugMode) {
        OutputDebug, % A_Now ": " message
        ToolTip, %message%, 0, 0
        SetTimer, RemoveToolTip, -2000
    }
}

RemoveToolTip:
    ToolTip
Return

; --- Toggle GUI visibility ---
ToggleGui() {
    global guiVisible
    if (guiVisible) {
        Gui, Hide
        guiVisible := false
    } else {
        Gui, Show, NA
        guiVisible := true
    }
}

; --- Drag handler for GUI ---
DragMove:
    PostMessage, 0xA1, 2,,, A
Return

; --- Button Handlers ---
SetLargeThumbs:
    CycleViewModeButton(1)
Return

SetTiles:
    CycleViewModeButton(2)
Return

SetDetails:
    CycleViewModeButton(3)
Return

SetList:
    CycleViewModeButton(4)
Return

SetCycleViewMode:
    CycleViewModeHotkey()
Return

; --- Cycle via Hotkey ---
CycleViewModeHotkey() {
    global viewModeIndex, viewModes
    viewModeIndex := Mod(viewModeIndex + 1, viewModes.MaxIndex())
    if (viewModeIndex = 0)
        viewModeIndex := viewModes.MaxIndex()
    mode := viewModes[viewModeIndex]
    CycleViewModeApply(mode)
}

; --- Cycle via Button ---
CycleViewModeButton(index) {
    global viewModeIndex, viewModes
    viewModeIndex := index
    mode := viewModes[viewModeIndex]
    CycleViewModeApply(mode)
}

; --- Get Active Explorer Window ---
GetActiveExplorerWindow() {
    DebugLog("Getting active Explorer window...")
    
    ; First, check if active window is Explorer
    WinGetClass, activeClass, A
    if (activeClass != "CabinetWClass" && activeClass != "ExploreWClass") {
        DebugLog("Active window is not Explorer: " activeClass)
        return false
    }
    
    ; Get active window HWND
    WinGet, activeHwnd, ID, A
    DebugLog("Active HWND: " activeHwnd)
    
    ; Try to find matching Explorer window via COM
    try {
        for window in ComObjCreate("Shell.Application").Windows {
            try {
                if (window && window.HWND = activeHwnd && window.Document) {
                    DebugLog("Found matching Explorer window via COM")
                    return window
                }
            }
        }
    } catch e {
        DebugLog("Error iterating windows: " e.Message)
    }
    
    DebugLog("No matching COM window found")
    return false
}

; --- Apply mode to Explorer ---
CycleViewModeApply(mode) {
    DebugLog("Attempting to apply mode: " mode)
    
    ; Method 1: Try active window first
    window := GetActiveExplorerWindow()
    if (window) {
        try {
            if (ApplyViewModeToWindow(window, mode)) {
                ShowNotification("View Mode: " mode)
                return
            }
        } catch e {
            DebugLog("Error applying to active window: " e.Message)
        }
    }
    
    ; Method 2: Try any Explorer window
    DebugLog("Trying any available Explorer window...")
    try {
        for window in ComObjCreate("Shell.Application").Windows {
            try {
                if (window && window.Document) {
                    if (ApplyViewModeToWindow(window, mode)) {
                        ShowNotification("View Mode: " mode " (any window)")
                        return
                    }
                }
            } catch e {
                DebugLog("Error with window: " e.Message)
                continue
            }
        }
    } catch e {
        DebugLog("Error creating Shell.Application: " e.Message)
    }
    
    ; Method 3: Fallback to keyboard shortcuts
    DebugLog("Falling back to keyboard shortcuts")
    UseFallbackKeystrokes(mode)
}

; --- Apply view mode to specific window ---
ApplyViewModeToWindow(window, mode) {
    try {
        oldMode := window.Document.CurrentViewMode
        DebugLog("Current view mode: " oldMode)
        
        if (mode = "LargeThumbs") {
            window.Document.CurrentViewMode := 1
            Sleep, 50  ; Small delay for the change to register
            window.Document.IconSize := 96
            DebugLog("Applied LargeThumbs mode")
        } else if (mode = "Tiles") {
            window.Document.CurrentViewMode := 6
            DebugLog("Applied Tiles mode")
        } else if (mode = "Details") {
            window.Document.CurrentViewMode := 4
            DebugLog("Applied Details mode")
        } else if (mode = "List") {
            window.Document.CurrentViewMode := 3
            DebugLog("Applied List mode")
        }
        
        ; Verify the change
        newMode := window.Document.CurrentViewMode
        DebugLog("New view mode: " newMode)
        return true
    } catch e {
        DebugLog("Error in ApplyViewModeToWindow: " e.Message)
        return false
    }
}

; --- Fallback keystroke method ---
UseFallbackKeystrokes(mode) {
    ; Ensure Explorer window is active
    WinGetClass, activeClass, A
    if (activeClass != "CabinetWClass" && activeClass != "ExploreWClass") {
        ; Try to activate any Explorer window
        WinActivate, ahk_class CabinetWClass
        Sleep, 100
    }
    
    if (mode = "LargeThumbs")
        Send ^+6
    else if (mode = "Tiles")
        Send ^+2
    else if (mode = "Details")
        Send ^+4
    else if (mode = "List")
        Send ^+3
    
    ShowNotification("Fallback: " mode)
    DebugLog("Used fallback keystrokes for: " mode)
}

; --- Show notification function ---
ShowNotification(message) {
    static notifGui := ""

    if (notifGui) {
        Gui, %notifGui%:Destroy
    }

    Gui, 99:+AlwaysOnTop -Caption +ToolWindow +E0x20
    Gui, 99:Color, 0x1E1E1E
    Gui, 99:Font, s9 c0xE0E0E0, Segoe UI
    Gui, 99:Margin, 15, 8
    Gui, 99:Add, Text, Center, %message%

    SysGet, MonitorWorkArea, MonitorWorkArea
    xPos := MonitorWorkAreaRight - 200
    yPos := MonitorWorkAreaBottom - 80
    Gui, 99:Show, x%xPos% y%yPos% NA

    notifGui := 99
    SetTimer, DestroyNotif, -1500
    Return

    DestroyNotif:
    Gui, 99:Destroy
    Return
}

; --- Custom Slider Class ---
class CustomSlider
{
    __New(Options := "", val := "", hBitmap_thumb := "", ShowTooltip := false) {
        Gui, Add, Progress, h3 %Options% hwndHPROG Disabled -E0x20000 c0x00BFFF Background0x00BFFF, % val
        Gui, Add, Text, xp yp-10 h24 wp BackgroundTrans HWNDhpgTrigger
        GuiControlGet, pg, Pos, %HPROG%
        x := pgX + (pgW * val / 100)
        Gui, Add, Progress, yp x%x% w8 h24 HWNDhBtn c0xFF0000 Background0xFF0000, 100
        
        this.hProg := HPROG
        this.hBtn := hBtn
        this.pgVal := val
        this.ShowTooltip := ShowTooltip
        this.lastVal := val

        fn := this.OnClick.Bind(this)
        GuiControl, +g, %hBtn%, %fn%
        GuiControl, +g, %hpgTrigger%, %fn%
    }

    OnClick() {
        GuiControl, MoveDraw, % this.hBtn
        GuiControl, Focus, % this.hBtn
        
        hSlider := this.hProg
        pre_CoordModeMouse := A_CoordModeMouse
        CoordMode, Mouse, Relative
        MouseGetPos,,,, ClickedhWnd, 2
        GuiControlGet, SliderLine, %A_Gui%:Pos, % hSlider
        GuiControlGet, sliderVal, %A_Gui%:, %hSlider%
        V := sliderVal

        while (GetKeyState("LButton")) {
            Sleep, 10
            MouseGetPos, XM, YM
            V := Round((XM - SliderLineX) / SliderLineW * 100)
            V := V < 1 ? 1 : V > 100 ? 100 : V
            if (V != this.lastVal) {
                this.lastVal := V
                this.pos := V
                if this.ShowTooltip
                    ToolTip % V
                this.ApplyToExplorer(V)
            }
        }
        if this.ShowTooltip
            ToolTip
        CoordMode, Mouse, %pre_CoordModeMouse%
    }

    ApplyToExplorer(newValue) {
        size := Round(16 + (newValue / 100) * (256 - 16))
        DebugLog("Slider: Applying size " size)
        
        ; Try active window first
        window := GetActiveExplorerWindow()
        if (window) {
            try {
                window.Document.CurrentViewMode := 1
                Sleep, 50
                window.Document.IconSize := size
                GuiControl,, SizeDisplay, % "Size: " size "px"
                ShowNotification("Thumbnail size: " size "px")
                DebugLog("Slider: Successfully applied size " size)
                return
            } catch e {
                DebugLog("Slider error with active window: " e.Message)
            }
        }
        
        ; Try any window
        for window in ComObjCreate("Shell.Application").Windows {
            try {
                if (window && window.Document) {
                    window.Document.CurrentViewMode := 1
                    Sleep, 50
                    window.Document.IconSize := size
                    GuiControl,, SizeDisplay, % "Size: " size "px"
                    ShowNotification("Thumbnail size: " size "px")
                    DebugLog("Slider: Applied to any window, size " size)
                    break
                }
            } catch e {
                DebugLog("Slider error: " e.Message)
                continue
            }
        }
    }

    pos {
        set {
            GuiControl,, % this.hProg, % value
            GuiControlGet, pg, Pos, % this.hProg
            x := pgX + (pgW * value / 100)
            GuiControl, MoveDraw, % this.hBtn, x%x%
            this.pgVal := value
        }
        get {
            return this.pgVal
        }
    }
}

; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_sliderthumb_png(NewHandle := False) {
Static hBitmap := 0
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 3864 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary", "Ptr", &B64, "UInt", 0, "UInt", 0x01, "Ptr", 0, "UIntP", DecLen, "Ptr", 0, "Ptr", 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary", "Ptr", &B64, "UInt", 0, "UInt", 0x01, "Ptr", &Dec, "UIntP", DecLen, "Ptr", 0, "Ptr", 0)
   Return False
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, "UPtr", DecLen, "UPtr")
pData := DllCall("Kernel32.dll\GlobalLock", "Ptr", hData, "UPtr")
DllCall("Kernel32.dll\RtlMoveMemory", "Ptr", pData, "Ptr", &Dec, "UPtr", DecLen)
DllCall("Kernel32.dll\GlobalUnlock", "Ptr", hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", "Ptr", hData, "Int", True, "PtrP", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", "UPtr")
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", "PtrP", pToken, "Ptr", &SI, "Ptr", 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  "Ptr", pStream, "PtrP", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", "Ptr", pBitmap, "PtrP", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", "Ptr", pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", "Ptr", pToken)
DllCall("Kernel32.dll\FreeLibrary", "Ptr", hGdip)
DllCall(NumGet(NumGet(pStream + 0, 0, "UPtr") + (A_PtrSize * 2), 0, "UPtr"), "Ptr", pStream)
Return hBitmap
}